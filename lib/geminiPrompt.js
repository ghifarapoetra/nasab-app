...
export function buildGeminiPrompt({ persons, marriages = [], treeName, treeDesc, sizeId, styleId, ownerName }) {
  ...
  const { hierarchy, flatList, relationships } = buildTreeHierarchy(persons, marriages)
...
function buildTreeHierarchy(persons, marriages = []) {
  ...
  const level = calculateGenerationLevels(persons, marriages)
