...
function drawTree(ctx, { persons, top, bottom, width, theme, size }) {
  ...
  const roots = persons.filter(p => (!p.father_id || !ids.has(p.father_id)) && (!p.mother_id || !ids.has(p.mother_id)))

  // ═══════════════════════════════════════════════════
  // Generation level — TOPOLOGICAL SORT approach
  // Level of a node = 1 + MAX(level of all its parents)
  // This ensures a child is always placed BELOW all its parents.
  // ═══════════════════════════════════════════════════
  const level = {}
  ...
  // Iteratively assign levels — keep looping until stable
  // Safe even with cycles thanks to max iterations
  let changed = true
  let iterations = 0
  ...
  while (changed && iterations < maxIter) {
    ...
    persons.forEach(p => {
      ...
      // Wait until all parents have levels assigned
      ...
      if (level[p.id] === undefined || level[p.id] < newLevel) {
        level[p.id] = newLevel
        changed = true
      }
    })
  ...
  // Fallback: any remaining orphans (broken refs) → level 0
...
function drawConnectors(ctx, persons, childrenMap, positions, theme, size) {
  ...
  persons.forEach(p => {
    ...
    kids.forEach(kidId => {
      ...
      ctx.beginPath()
      ctx.moveTo(from.cx, from.bottom)
      ctx.lineTo(from.cx, midY)
      ctx.lineTo(to.cx, midY)
      ctx.lineTo(to.cx, to.top)
      ctx.stroke()
    })
  })
  ctx.restore()
