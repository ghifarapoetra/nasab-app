...
export default function PdfThemeModal({ treeName, memberCount, isPremium, onClose }) {
  ...
  async function handleExport() {
    ...
    try {
      setProgress('Mengambil gambar pohon...')
      await new Promise(r => setTimeout(r, 80)) // allow UI to update
      setProgress('Menggambar poster...')
      await exportPoster({ themeId: selected, treeName, memberCount, isPremium, format })
      setProgress('Selesai!')
      setTimeout(() => { setExporting(false); setProgress('') }, 1200)
    } catch (e) {
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:16 }}>
      <div style={{ background:'var(--card)',borderRadius:18,padding:24,maxWidth:640,width:'100%',maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 60px rgba(0,0,0,.3)' }}>
        ...
        <div style={{ display:'flex',gap:8,margin:'14px 0 16px' }}>
          {[
            { id:'pdf', label:'📄 PDF', desc:'Siap cetak' },
            { id:'png', label:'🖼️ PNG', desc:'Resolusi tinggi' },
          ].map(f => (
        ...
        {/* Free themes */}
        <div style={{ fontSize:10,fontWeight:700,color:'var(--tx3)',letterSpacing:'.8px',textTransform:'uppercase',marginBottom:8 }}>
          Tema Gratis (5)
        </div>
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16 }}>
          {free.map(t => (
            <ThemeCard key={t.id} theme={t} selected={selected} onSelect={setSelected} locked={false} />
          ))}
        </div>

        {/* Premium themes */}
        <div style={{ display:'flex',alignItems:'center',gap:8,marginBottom:8 }}>
          <div style={{ fontSize:10,fontWeight:700,color:'var(--tx3)',letterSpacing:'.8px',textTransform:'uppercase' }}>
            Tema Premium (5)
          </div>
          {!isPremium && (
            <span style={{ fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:20,background:'var(--amber-bg)',color:'var(--amber-t)',border:'1px solid var(--amber-b)' }}>
              ✨ Upgrade Rp 29k
            </span>
          )}
        </div>
        <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:20 }}>
          {premium.map(t => (
            <ThemeCard key={t.id} theme={t} selected={selected} onSelect={setSelected} locked={!isPremium} />
          ))}
        </div>
        ...
        <div style={{ background:'var(--surf)',border:'1px solid var(--bd)',borderRadius:10,padding:'10px 14px',marginBottom:16,fontSize:12,color:'var(--tx2)',lineHeight:1.7 }}>
          💡 Output berukuran <strong>A3 landscape beresolusi tinggi</strong> — siap dicetak di print shop atau dipasang sebagai figura. Watermark <strong>sulalah.my.id</strong> muncul di bagian bawah.
        </div>

        {err && (
          <div style={{ background:'var(--rose-bg)',border:'1px solid var(--rose-b)',borderRadius:8,padding:'8px 12px',marginBottom:12,fontSize:12,color:'var(--rose-t)' }}>
            ⚠ {err}
          </div>
        )}
        ...
        <div style={{ display:'flex',gap:8,justifyContent:'flex-end' }}>
          ...
          <button className="btn btn-primary btn-pill" onClick={handleExport} disabled={exporting}
            style={{ minWidth:180,justifyContent:'center',fontSize:14,padding:'10px 24px',position:'relative' }}>
            {exporting ? (
              <span style={{ display:'flex',alignItems:'center',gap:8 }}>
                <Spinner /> {progress || 'Memproses...'}
              </span>
            ) : (
  ...
  return (
    <div onClick={() => !locked && onSelect(theme.id)}
      style={{ display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,
        border:`2px solid ${isSel?'var(--t4)':'var(--bd)'}`,
        background:isSel?'var(--surf)':'transparent',
        cursor:locked?'not-allowed':'pointer',
        opacity:locked?0.65:1,
        transition:'all .15s' }}>
      {/* Color swatch */}
      <div style={{ width:44,height:36,borderRadius:7,background:theme.previewBg,border:`1px solid ${theme.previewBorder}`,flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',overflow:'hidden',position:'relative' }}>
        <div style={{ width:8,height:8,borderRadius:'50%',background:theme.previewAccent,boxShadow:`0 0 6px ${theme.previewAccent}` }} />
        {locked && (
          <div style={{ position:'absolute',inset:0,background:'rgba(0,0,0,.35)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14 }}>🔒</div>
        )}
      </div>
